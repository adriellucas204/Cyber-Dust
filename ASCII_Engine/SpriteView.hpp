...
public:
  SpriteView(SpriteBase &spbase, int lin, int col, int height, int width,
             COR::Cor cor = COR::NULL_COLOR);
  // a partir de qual linha e coluna meu spritebase vai ser desenhado, e as
  // outras duas dimensoes sao para o tamanho da janela do spriteview
  ~SpriteView() = default;
  ...
  virtual void draw(SpriteBase &screen, int x, int y) {
    screen.putAt(*this, x + col, y + lin); // foi alterado(original: x, y - col)
  }
