...
public:
  enum {
    ...
    LEVEL_5, // fases
    PLAYING,
    PAUSED,         // opcoes para a fase
    LEVEL_COMPLETE, // fase completada
    END_GAME        // quando jogador nao quiser mais jogar
  };

  Fase(std::string name, const SpriteBase &bkg)
      : name(name), background(bkg.copia()) {}
  virtual ~Fase();
  ...
  virtual unsigned
  run(SpriteBuffer &screen) = 0; // cada derivada tem de implementar sua lógica
                                 // nesse método
